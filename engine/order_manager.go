...
func SetupOrderManager(exchangeManager iExchangeManager, wg *sync.WaitGroup, verbose, activelyTrackFuturesPositions bool, futuresTrackingSeekDuration time.Duration) (*OrderManager, error) {
...
func (m *OrderManager) Modify(ctx context.Context, mod *order.Modify) (*order.ModifyResponse, error) {
	...
	if err != nil {
		...
		log.Debugln(log.OrderMgr, message)
		return nil, err
	}
