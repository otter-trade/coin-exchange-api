...
func TestSetupOrderManager(t *testing.T) {
	_, err := SetupOrderManager(nil, nil, false, false, 0)
	if !errors.Is(err, errNilExchangeManager) {
		t.Errorf("error '%v', expected '%v'", err, errNilExchangeManager)
	}
	_, err = SetupOrderManager(NewExchangeManager(), nil, false, false, 0)
	...
	_, err = SetupOrderManager(NewExchangeManager(), &wg, false, false, 0)
	if !errors.Is(err, nil) {
		t.Errorf("error '%v', expected '%v'", err, nil)
	}
	_, err = SetupOrderManager(NewExchangeManager(), &wg, false, true, 0)
	if !errors.Is(err, nil) {
		t.Errorf("error '%v', expected '%v'", err, nil)
	}
	_, err = SetupOrderManager(NewExchangeManager(), &wg, false, true, 1337)
	if !errors.Is(err, nil) {
		t.Errorf("error '%v', expected '%v'", err, nil)
	}
}
...
func TestOrderManagerStart(t *testing.T) {
	...
	var wg sync.WaitGroup
	m, err = SetupOrderManager(NewExchangeManager(), &wg, false, false, 0)
	if !errors.Is(err, nil) {
		t.Errorf("error '%v', expected '%v'", err, nil)
	}
	err = m.Start()
...
func OrdersSetup(t *testing.T) *OrderManager {
	...
	m, err := SetupOrderManager(em, &wg, false, false, 0)
	if !errors.Is(err, nil) {
		t.Errorf("error '%v', expected '%v'", err, nil)
	}
	m.started = 1
